...
from ecom_backend_framework.app.services.utility.wishlist_service import WishlistService
...
async def get_user_wishlist(current_user: dict = Depends(get_current_user)):
    result = await WishlistService.get_wishlist(current_user['user_id'])
    return result
...
async def add_item_to_wishlist(
    ...
    result = await WishlistService.add_item(current_user['user_id'], wishlist_item)
    return result
...
async def update_wishlist_item(
    ...
    result = await WishlistService.update_item(current_user['user_id'], product_id, wishlist_update)
    return result
...
async def delete_item_from_wishlist(
    ...
    await WishlistService.delete_item(current_user['user_id'], product_id)
    return None
...
async def move_wishlist_to_cart(
    ...
    result = await WishlistService.move_to_cart(current_user['user_id'], product_id, move_data)
    return result